...
describe('Goldstack API', () => {
  it('Create a project', async () => {
    ...
    const session = (0, supertest_session_1.default)(server_1.app, { cookieAccess });
    const sessionRes = await session.post('/sessions').send();
    expect(sessionRes.status).toEqual(200);
    const cookie = session.cookies.find((cookie) => cookie.name === 'userToken');
    ...
    const invalidRequest = await (0, supertest_1.default)(server_1.app)
