...
const buildPackage = async (packageId, data) => {
    const packageRes = await fetch(`${getEndpoint()}/projects/${data.projectId}/packages/${packageId}`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
...
const ProjectConfigSummary = (props) => {
    ...
    const clickBuildProject = async () => {
        ...
        try {
            Fullstory.setUserVars({
                selectedTemplates_strs: props.projectData.packageConfigs.map((config) => config.package.template),
            });
        }
        catch (e) {
    ...
    return (<>
      {validationResult.map((result, idx) => (<ValidationResult result={result} key={idx}></ValidationResult>))}
      {!allValid && <p>Please complete the configuration for the steps marked with ❌ above.</p>}

      {allValid && (<p>Click the button below to build the downloadable archive for your project.</p>)}
      <Row className="space-top-2 space-bottom-2">
        <Col xs={3}></Col>
        <Col xs={6} className="text-center">
          <BuildProjectButton disabled={!allValid || progressMessage !== undefined} onClick={clickBuildProject} type="button" className="btn btn-primary btn-lg transition-3d-hover">
            <Spinner as="span" animation="border" role="status" aria-hidden="true" hidden={!progressMessage}></Spinner>{' '}
            Build Project 🛠️
          </BuildProjectButton>
        </Col>
        <Col xs={3}>
...
//# sourceMappingURL=ProjectConfigSummary.jsx.map