...
const IconList = (props) => {
    ...
    return (<div className="row justify-content-center">
      {images.map((image, idx) => (<div className={'col-4 col-sm-3 my-2 '} key={idx}>
...
const createImage = (image) => {
    ...
    if (image === 'email-send') {
        // biome-ignore lint/suspicious/noExplicitAny: PNG import requires type assertion
        return <img src={EmailSendImg} className="img-fluid"></img>;
    }
    throw new Error(`Unknown image for feature details ${image}`);
};
const TemplateFeature = (props) => {
    if (!props.details) {
        return <></>;
    }
    ...
    switch (details.content.type) {
        ...
        case 'combine-templates': {
            content = (<FeatureCombineTemplates templates={details.content.data.templates}></FeatureCombineTemplates>);
            break;
        }
        case 'project-install': {
            content = (<FeatureProjectInstall projectName={details.content.data.projectName}></FeatureProjectInstall>);
            break;
        }
        case 'bootstrap': {
    ...
    return (<div className="position-relative gradient-y-gray">
      <div className="container space-2 space-bottom-sm-3">
        <div className="w-md-80 w-lg-50 text-center mx-md-auto mb-5 mb-md-5">
          <div id={props.id || ''}>
            <span className="d-block small font-weight-bold text-cap mb-2" style={{ cursor: 'pointer' }} onClick={() => window.history.back()}>
              {props.title} ⤴
            </span>
          </div>
          <h2>{details.title}</h2>
        ...
        <div className=" text-center mx-md-auto mb-5 mb-md-5">
          {content}
          {details.moreDetails && (<div className="w-md-80 w-lg-50 mx-md-auto text-center">
        ...
        {details.callToAction && (<div className="text-center">
            <a className="btn btn-primary transition-3d-hover px-lg-7" href={details.callToAction.link}>
              <span className={styles['plus-icon']} dangerouslySetInnerHTML={{ __html: plusSvg }}></span>{' '}
...
//# sourceMappingURL=TemplateFeature.jsx.map