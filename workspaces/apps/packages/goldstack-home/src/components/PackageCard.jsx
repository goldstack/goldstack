...
const PackageCard = (props) => {
    ...
    return (<div className={`card card-frame h-100 ${cardCss} `}>
          ...
          <div className="media-body">
            ...
            <span className={'d-block text-body'}>
              ...
              <ul className={styles['feature-list']}>
                {props.features.map((feature, index) => (<li key={index}>{feature.name}</li>))}
              </ul>
            </span>
          </div>
        </div>
      </div>
      {!included && !props.alwaysIncluded && (<div className={`card-footer btn btn-primary btn-sm ${styles['include-footer']} ${props.disabled ? 'disabled' : ''}`} onClick={toggleIncluded}>
          <span className={styles.icon} dangerouslySetInnerHTML={{ __html: plusSvg }}></span>
          Add to Project
        </div>)}
      {included && !props.alwaysIncluded && (<div className={`card-footer btn btn-success btn-sm ${styles['included-footer']}`} onClick={toggleIncluded}>
          <span className={styles.icon} dangerouslySetInnerHTML={{ __html: checkSvg }}></span>
          Included in Project
        </div>)}
      {props.alwaysIncluded && false && (<div className={`card-footer text-center btn-sm ${styles['always-included-footer']}`}>
          <span className={styles.icon} dangerouslySetInnerHTML={{ __html: checkSvg }}></span>
          Included in Project
        </div>)}
    </div>);
...
//# sourceMappingURL=PackageCard.jsx.map