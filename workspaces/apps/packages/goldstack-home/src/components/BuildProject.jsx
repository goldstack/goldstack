...
export const BuildProject = (params) => {
    ...
    const doConfigure = async (overrideWarning) => {
        if (!overrideWarning) {
    ...
    return (<>
      <Row>
        <PackageList selectedPackages={selectedPackages} items={packages} disabled={progressMessage !== ''} onDeselect={(packageId) => {
      ...
      <Row className="space-top-2 space-bottom-2">
        ...
        <Col xs={4} className="text-center">
          <ConfigureProjectButton disabled={progressMessage !== ''} onClick={clickConfigure} type="button" className="btn btn-primary btn-lg transition-3d-hover">
            <Spinner as="span" animation="border" role="status" aria-hidden="true" hidden={!progressMessage} className={styles.spinner}></Spinner>{' '}
            Configure Project 🛠️
...
//# sourceMappingURL=BuildProject.jsx.map