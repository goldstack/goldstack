...
import { read, write, mkdir, copy, rmSafe } from '@goldstack/utils-sh';
  ...
  class GeneratedBuilderConfig implements PrepareTemplate {
    ...
    async run(params: PrepareTemplateParams): Promise<void> {
      ...
      for (const glob of buildConfig.include) {
        ...
        const dest = params.destinationDirectory + dirComponent;
        mkdir('-p', dest);

        const source = templateDirectory + glob;

        await copy(source, dest);
      }

      await rmSafe(
        ...buildConfig.exclude.map((glob) => params.destinationDirectory + glob)
      );
